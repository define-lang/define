...
/-!
This aggregate currently contains the two concrete models that still use the
legacy `ResolvedDefineGraph` interface. They demonstrate selected semantic
obligations but supply no premise to the minimality theorem. Each will move to
...
The two namespaces below still construct the older `ResolvedDefineGraph`
interface directly; they remain to be migrated to the same end-to-end path as
the imported witnesses.
-/
