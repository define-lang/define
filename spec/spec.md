...
In Idiomatic Define, local names only contain `lowercase_ascii_with_underscore`.
...
Because names can map to directory paths, directory path segments on the
filesystem follow the same naming rules as names, except that directory names
may not contain `/` by default. When matching filesystem paths to global names,
if a filesystem path contains Unicode codepoints subject to normalization, it
must be normalized to Normalization Form C internally in the compiler before
comparing it to the global name.
...
A universe name may prefix a global name to disambiguate where that name is
defined. When present, the universe name is the substring before the first `:`
character in the global name, and the `:` character separates the universe name
from the rest of the global name.

Universe names are restricted to lowercase ASCII letters and `_`. Universe names
must not contain `:`, `/`, or `@`.

```ebnf
universe_name = universe_name_char, { universe_name_char } ;
universe_name_char = lowercase_ascii | "_" ;
```

Universe names are case-sensitive. Reserved universe names are case-insensitive.

The compiler maintains a fixed list of reserved universe names. This list must
include:
...
The list also includes common English words defined by the compiler. Tools that
are part of the Define language must refuse to download, create, or interact
with universes that have reserved names, except where explicitly allowed by this
specification.

When compiling code from files on a filesystem, the compiler must locate a
configuration file that deterministically identifies the current universe name
and its root directory. If such a configuration file exists, all definitions in
the referenced files must use that universe name. Non-filesystem compilation
contexts may omit this configuration requirement.

If two different codebases claim the same universe name in the same compilation
context, the compiler must fail with an error indicating the conflict.
