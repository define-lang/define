...
_DEFAULT_FILTERS = AnalysisFilters()
...
class StackPathRow:
    ...
    stack_path: tuple[FunctionIdentity, ...]
    wall_occupancy_ns: int
    thread_time_ns: int
    longest_span_ns: int
    sample_hits: int
...
def _is_compiler_filename(filename: str) -> bool:
...
def _is_compiler_frame(frame: schema.Frame) -> bool:
    return _is_compiler_filename(frame["filename"])


def _matches_frame(frame: schema.Frame, filters: AnalysisFilters) -> bool:
    ...
    if filters.function is not None and filters.function not in frame["function"]:
        return False
    return not filters.compiler_only or _is_compiler_frame(frame)


def _function_identity(frame: schema.Frame) -> FunctionIdentity:
...
def _matches_function(identity: FunctionIdentity, filters: AnalysisFilters) -> bool:
    ...
    if filters.function is not None and filters.function not in identity.function:
        return False
    if not filters.compiler_only:
        return True
    return _is_compiler_filename(identity.filename)
...
def _frame_rows(
    ...
    for frame_id, entries in entries_by_frame.items():
        ...
        if not _matches_frame(frame, filters):
            continue
...
def _function_rows(
    ...
    hits_by_function: dict[FunctionIdentity, int],
    filters: AnalysisFilters,
) -> list[FunctionRow]:
    rows: list[FunctionRow] = []
    for identity, entries in entries_by_function.items():
        if not _matches_function(identity, filters):
            continue
        intervals = [interval for _, interval in entries]
        rows.append(
            FunctionRow(
...
def _stack_path_rows(
    entries_by_path: dict[
        tuple[FunctionIdentity, ...],
        list[tuple[int, _Interval]],
    ],
    hits_by_path: dict[tuple[FunctionIdentity, ...], int],
    filters: AnalysisFilters,
) -> list[StackPathRow]:
    # PRF-015: Full stacks.
    rows: list[StackPathRow] = []
    for stack_path, entries in entries_by_path.items():
        if not any(_matches_function(identity, filters) for identity in stack_path):
            continue
        intervals = [interval for _, interval in entries]
        rows.append(
            StackPathRow(
...
def analyze(
    ...
    filters: AnalysisFilters = _DEFAULT_FILTERS,
) -> Analysis:
    ...
    relationship_entries: dict[
        tuple[FunctionIdentity, FunctionIdentity],
        list[tuple[int, _Interval]],
    ] = {}
    self_hits: dict[int, int] = {}
    cumulative_hits: dict[int, int] = {}
    self_function_hits: dict[FunctionIdentity, int] = {}
    cumulative_function_hits: dict[FunctionIdentity, int] = {}
    stack_path_hits: dict[tuple[FunctionIdentity, ...], int] = {}
    relationship_hits: dict[tuple[FunctionIdentity, FunctionIdentity], int] = {}
    ...
    for weighted_stack in weighted_stacks:
        ...
        leaf_function = _function_identity(frames[leaf])
        ...
        function_identities = {
            _function_identity(frames[frame_id]) for frame_id in weighted_stack.stack
        }
    ...
    for relationship, entries in relationship_entries.items():
        ...
        if filters.callee is not None and filters.callee not in callee.function:
            continue
        if not _matches_function(caller, filters) and not _matches_function(
            callee,
            filters,
        ):
            continue
...
def _function_text(identity: FunctionIdentity) -> str:
...
@click.command(
    epilog=(
        "Wall rows report unioned process occupancy and non-additive thread time. "
        "A sample hit is not a function call."
    )
)
@click.option(
    "--profile",
...
def main(
    ...
    *,
    ...
    """Analyze one continuous raw wall profile."""
    ...
    emit_report(profile, analyze(profile, filters), limit)
