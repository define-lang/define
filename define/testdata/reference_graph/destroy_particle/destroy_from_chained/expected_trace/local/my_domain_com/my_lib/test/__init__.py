"""Generated by the Define compiler."""

from typing import ClassVar, override

from define.runtime import literal

import local.my_domain_com.my_lib.act
import local.my_domain_com.my_lib.end
import local.my_domain_com.my_lib.mid


class Test(literal.Action):
    implied_qualities: ClassVar[tuple[type[literal.Quality], ...]] = (
        local.my_domain_com.my_lib.act.Act,
    )

    @override
    def run(self):
        self.on_particle.get_action(
            local.my_domain_com.my_lib.act.Act
        ).get_interface_position(
            "position<chain_src>"
        ).create_particle()
        literal.record_operation("test.create(/act::chain_src)")
        self.on_particle.get_action(
            local.my_domain_com.my_lib.act.Act
        ).get_interface_position(
            "position<chain_src>"
        ).particle.get_position(
            local.my_domain_com.my_lib.mid.Mid
        ).create_particle()
        literal.record_operation("test.create(/act::chain_src::/mid)")
        self.on_particle.get_action(
            local.my_domain_com.my_lib.act.Act
        ).get_interface_position(
            "position<chain_src>"
        ).particle.get_position(
            local.my_domain_com.my_lib.mid.Mid
        ).particle.get_position(
            local.my_domain_com.my_lib.end.End
        ).create_particle()
        literal.record_operation("test.create(/act::chain_src::/mid::/end)")
        self.on_particle.get_action(
            local.my_domain_com.my_lib.act.Act
        ).get_interface_position(
            "position<trigger>"
        ).create_particle()
        literal.record_operation("test.create(/act::trigger)")
        self.on_particle.get_action(
            local.my_domain_com.my_lib.act.Act
        ).run()
