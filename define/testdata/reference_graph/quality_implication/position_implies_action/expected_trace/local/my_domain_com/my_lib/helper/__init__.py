"""Generated by the Define compiler."""

from typing import override

from define.runtime import literal


class HelperDestructionContracts:
    def run_destructors_position_run(self, _particle: literal.Particle):
        pass

    def destroy_position_run(self, _particle: literal.Particle):
        pass


_DEFAULT_DESTRUCTION_CONTRACTS = HelperDestructionContracts()


class Helper(literal.Action):

    def __init__(self, on_particle: literal.Particle):
        super().__init__(
            on_particle,
            interface_positions=[
                literal.LocalPosition("position<run>"),
            ],
        )

    @override
    def run(self, destruction_contracts: HelperDestructionContracts = _DEFAULT_DESTRUCTION_CONTRACTS):
        work = literal.LocalPosition(
            "position<work>",
        )
        work.create_particle()
        literal.record_operation("helper.create(work)")
        destruction_contracts.run_destructors_position_run(
            self.get_interface_position(
                "position<run>"
            ).particle
        )
        destruction_contracts.destroy_position_run(
            self.get_interface_position(
                "position<run>"
            ).particle
        )
        self.get_interface_position(
            "position<run>"
        ).destroy_particle()
        literal.record_operation("helper.destroy(run)")
        work.destroy_particle()
        literal.record_operation("helper.destroy(work)")
