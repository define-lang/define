"""Generated by the Define compiler."""

from typing import override

from define.runtime import literal

import local.my_domain_com.my_lib.implier
import local.my_domain_com.my_lib.middle


class RunnerDestructionContracts:
    def run_destructors_position_wrapper(self, _particle: literal.Particle):
        pass

    def destroy_position_wrapper(self, _particle: literal.Particle):
        pass

    def run_destructors_position_run(self, _particle: literal.Particle):
        pass

    def destroy_position_run(self, _particle: literal.Particle):
        pass


_DEFAULT_DESTRUCTION_CONTRACTS = RunnerDestructionContracts()


class Runner(literal.Action):

    def __init__(self, on_particle: literal.Particle):
        super().__init__(
            on_particle,
            interface_positions=[
                literal.LocalPosition("position<run>"),
                literal.LocalPosition(
                    "position<wrapper>",
                    constraints=(
                        local.my_domain_com.my_lib.middle.Middle,
                    ),
                ),
            ],
        )

    @override
    def run(self, destruction_contracts: RunnerDestructionContracts = _DEFAULT_DESTRUCTION_CONTRACTS):
        self.get_interface_position(
            "position<wrapper>"
        ).particle.get_action(
            local.my_domain_com.my_lib.middle.Middle
        ).get_interface_position(
            "position<box>"
        ).create_particle()
        literal.record_operation("runner.create(wrapper::/middle::box)")
        self.get_interface_position(
            "position<wrapper>"
        ).particle.get_action(
            local.my_domain_com.my_lib.middle.Middle
        ).get_interface_position(
            "position<box>"
        ).particle.get_action(
            local.my_domain_com.my_lib.implier.Implier
        ).run()
        self.get_interface_position(
            "position<wrapper>"
        ).particle.get_action(
            local.my_domain_com.my_lib.middle.Middle
        ).get_interface_position(
            "position<run>"
        ).create_particle()
        literal.record_operation("runner.create(wrapper::/middle::run)")
        self.get_interface_position(
            "position<wrapper>"
        ).particle.get_action(
            local.my_domain_com.my_lib.middle.Middle
        ).run()
        destruction_contracts.run_destructors_position_wrapper(
            self.get_interface_position(
                "position<wrapper>"
            ).particle
        )
        destruction_contracts.destroy_position_wrapper(
            self.get_interface_position(
                "position<wrapper>"
            ).particle
        )
        self.get_interface_position(
            "position<wrapper>"
        ).destroy_particle()
        literal.record_operation("runner.destroy(wrapper)")
        destruction_contracts.run_destructors_position_run(
            self.get_interface_position(
                "position<run>"
            ).particle
        )
        destruction_contracts.destroy_position_run(
            self.get_interface_position(
                "position<run>"
            ).particle
        )
        self.get_interface_position(
            "position<run>"
        ).destroy_particle()
        literal.record_operation("runner.destroy(run)")
