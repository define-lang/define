"""Generated by the Define compiler."""

from typing import override

from define.runtime import literal

import local.my_domain_com.my_lib.final


class ReactADestructionContracts:
    def run_destructors_position_trigger(self, _particle: literal.Particle):
        pass

    def destroy_position_trigger(self, _particle: literal.Particle):
        pass


_DEFAULT_DESTRUCTION_CONTRACTS = ReactADestructionContracts()


class ReactA(literal.Action):

    def __init__(self, on_particle: literal.Particle):
        super().__init__(
            on_particle,
            interface_positions=[
                literal.LocalPosition("position<trigger>"),
                literal.LocalPosition(
                    "position<result>",
                    constraints=(
                        local.my_domain_com.my_lib.final.Final,
                    ),
                ),
            ],
        )

    @override
    def run(self, destruction_contracts: ReactADestructionContracts = _DEFAULT_DESTRUCTION_CONTRACTS):
        self.get_interface_position(
            "position<result>"
        ).create_particle()
        literal.record_operation("react_a.create(result)")
        self.get_interface_position(
            "position<result>"
        ).particle.get_action(
            local.my_domain_com.my_lib.final.Final
        ).get_interface_position(
            "position<trigger>"
        ).create_particle()
        literal.record_operation("react_a.create(result::/final::trigger)")
        self.get_interface_position(
            "position<result>"
        ).particle.get_action(
            local.my_domain_com.my_lib.final.Final
        ).run()
        self.get_interface_position(
            "position<result>"
        ).destroy_particle()
        literal.record_operation("react_a.destroy(result)")
        destruction_contracts.run_destructors_position_trigger(
            self.get_interface_position(
                "position<trigger>"
            ).particle
        )
        destruction_contracts.destroy_position_trigger(
            self.get_interface_position(
                "position<trigger>"
            ).particle
        )
        self.get_interface_position(
            "position<trigger>"
        ).destroy_particle()
        literal.record_operation("react_a.destroy(trigger)")
