"""Generated by the Define compiler."""

from typing import override

from define.runtime import literal

import local.my_domain_com.my_lib.react_a
import local.my_domain_com.my_lib.react_b


class KickOffDestructionContracts:
    def run_destructors_position_trigger(self, _particle: literal.Particle):
        pass

    def destroy_position_trigger(self, _particle: literal.Particle):
        pass


_DEFAULT_DESTRUCTION_CONTRACTS = KickOffDestructionContracts()


class KickOff(literal.Action):

    def __init__(self, on_particle: literal.Particle):
        super().__init__(
            on_particle,
            interface_positions=[
                literal.LocalPosition("position<trigger>"),
                literal.LocalPosition(
                    "position<output>",
                    constraints=(
                        local.my_domain_com.my_lib.react_a.ReactA,
                        local.my_domain_com.my_lib.react_b.ReactB,
                    ),
                ),
            ],
        )

    @override
    def run(self, destruction_contracts: KickOffDestructionContracts = _DEFAULT_DESTRUCTION_CONTRACTS):
        self.get_interface_position(
            "position<output>"
        ).create_particle()
        literal.record_operation("kick_off.create(output)")
        self.get_interface_position(
            "position<output>"
        ).particle.get_action(
            local.my_domain_com.my_lib.react_a.ReactA
        ).get_interface_position(
            "position<trigger>"
        ).create_particle()
        literal.record_operation("kick_off.create(output::/react_a::trigger)")
        self.get_interface_position(
            "position<output>"
        ).particle.get_action(
            local.my_domain_com.my_lib.react_a.ReactA
        ).run()
        self.get_interface_position(
            "position<output>"
        ).particle.get_action(
            local.my_domain_com.my_lib.react_b.ReactB
        ).get_interface_position(
            "position<trigger>"
        ).create_particle()
        literal.record_operation("kick_off.create(output::/react_b::trigger)")
        self.get_interface_position(
            "position<output>"
        ).particle.get_action(
            local.my_domain_com.my_lib.react_b.ReactB
        ).run()
        self.get_interface_position(
            "position<output>"
        ).destroy_particle()
        literal.record_operation("kick_off.destroy(output)")
        destruction_contracts.run_destructors_position_trigger(
            self.get_interface_position(
                "position<trigger>"
            ).particle
        )
        destruction_contracts.destroy_position_trigger(
            self.get_interface_position(
                "position<trigger>"
            ).particle
        )
        self.get_interface_position(
            "position<trigger>"
        ).destroy_particle()
        literal.record_operation("kick_off.destroy(trigger)")
