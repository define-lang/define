...
class Middle(literal.Action):
    ...
    def run(self, destruction_contracts: MiddleDestructionContracts = _DEFAULT_DESTRUCTION_CONTRACTS):
        ...
        literal.record_operation("middle.create(/destroyer::trigger_pos)")
        self.on_particle.get_action(
            local.my_domain_com.my_lib.destroyer.Destroyer
        ).run(
...
class DestroyerDestructionContracts(local.my_domain_com.my_lib.destroyer.DestroyerDestructionContracts):
    ...
    def destroy_global_position_parent(self, particle: literal.Particle):
        ...
        literal.record_operation("destroyer.destroy(/parent::/second)")
        particle.get_position(
            local.my_domain_com.my_lib.first.First
        ).destroy_particle()
        literal.record_operation("destroyer.destroy(/parent::/first)")
