"""Generated by the Define compiler."""

from typing import ClassVar, override

from define.runtime import literal

import local.my_domain_com.my_lib.destination
import local.my_domain_com.my_lib.known_empty
import local.my_domain_com.my_lib.known_occupied
import local.my_domain_com.my_lib.target


class DestroyerDestructionContracts:
    def run_destructors_position_run(self, _particle: literal.Particle):
        pass

    def destroy_position_run(self, _particle: literal.Particle):
        pass


_DEFAULT_DESTRUCTION_CONTRACTS = DestroyerDestructionContracts()


class Destroyer(literal.Action):
    implied_qualities: ClassVar[tuple[type[literal.Quality], ...]] = (
        local.my_domain_com.my_lib.destination.Destination,
        local.my_domain_com.my_lib.target.Target,
    )

    def __init__(self, on_particle: literal.Particle):
        super().__init__(
            on_particle,
            interface_positions=[
                literal.LocalPosition(
                    "position<run>",
                    constraints=(
                        local.my_domain_com.my_lib.known_empty.KnownEmpty,
                        local.my_domain_com.my_lib.known_occupied.KnownOccupied,
                    ),
                ),
            ],
        )

    @override
    def run(self, destruction_contracts: DestroyerDestructionContracts = _DEFAULT_DESTRUCTION_CONTRACTS):
        local_ = literal.LocalPosition(
            "position<local>",
            constraints=(
                local.my_domain_com.my_lib.known_empty.KnownEmpty,
                local.my_domain_com.my_lib.known_occupied.KnownOccupied,
            ),
        )
        self.get_interface_position(
            "position<run>"
        ).move_particle_to(
            self.on_particle.get_position(
                local.my_domain_com.my_lib.target.Target
            )
        )
        literal.record_operation("destroyer.move(run, /target)")
        self.on_particle.get_position(
            local.my_domain_com.my_lib.target.Target
        ).move_particle_to(
            local_
        )
        literal.record_operation("destroyer.move(/target, local)")
        local_.particle.get_position(
            local.my_domain_com.my_lib.known_empty.KnownEmpty
        ).move_particle_to(
            self.on_particle.get_position(
                local.my_domain_com.my_lib.destination.Destination
            )
        )
        literal.record_operation("destroyer.move(local::/known_empty, /destination)")
        local_.particle.get_position(
            local.my_domain_com.my_lib.known_occupied.KnownOccupied
        ).create_particle()
        literal.record_operation("destroyer.create(local::/known_occupied)")
        destruction_contracts.run_destructors_position_run(
            local_.particle
        )
        destruction_contracts.destroy_position_run(
            local_.particle
        )
        local_.particle.get_position(
            local.my_domain_com.my_lib.known_occupied.KnownOccupied
        ).destroy_particle()
        literal.record_operation("destroyer.destroy(local::/known_occupied)")
        local_.destroy_particle()
        literal.record_operation("destroyer.destroy(local)")
