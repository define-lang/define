"""Generated by the Define compiler."""

from typing import override

from define.runtime import literal

import local.my_domain_com.my_lib.child
import local.my_domain_com.my_lib.destroyer
import local.my_domain_com.my_lib.sibling


class MiddleBDestructionContracts:
    def run_destructors_position_run(self, _particle: literal.Particle):
        pass

    def destroy_position_run(self, _particle: literal.Particle):
        pass


_DEFAULT_DESTRUCTION_CONTRACTS = MiddleBDestructionContracts()


class MiddleB(literal.Action):

    def __init__(self, on_particle: literal.Particle):
        super().__init__(
            on_particle,
            interface_positions=[
                literal.LocalPosition("position<run>"),
            ],
        )

    @override
    def run(self, destruction_contracts: MiddleBDestructionContracts = _DEFAULT_DESTRUCTION_CONTRACTS):
        box = literal.LocalPosition(
            "position<box>",
            constraints=(
                local.my_domain_com.my_lib.child.Child,
                local.my_domain_com.my_lib.sibling.Sibling,
            ),
        )
        destroyer_holder = literal.LocalPosition(
            "position<destroyer_holder>",
            constraints=(
                local.my_domain_com.my_lib.destroyer.Destroyer,
            ),
        )
        destroyer_holder.create_particle()
        literal.record_operation("middle_b.create(destroyer_holder)")
        box.create_particle()
        literal.record_operation("middle_b.create(box)")
        box.particle.get_position(
            local.my_domain_com.my_lib.sibling.Sibling
        ).create_particle()
        literal.record_operation("middle_b.create(box::/sibling)")
        box.particle.get_position(
            local.my_domain_com.my_lib.child.Child
        ).create_particle()
        literal.record_operation("middle_b.create(box::/child)")
        box.move_particle_to(
            destroyer_holder.particle.get_action(
                local.my_domain_com.my_lib.destroyer.Destroyer
            ).get_interface_position(
                "position<run>"
            )
        )
        literal.record_operation("middle_b.move(box, destroyer_holder::/destroyer::run)")
        destroyer_holder.particle.get_action(
            local.my_domain_com.my_lib.destroyer.Destroyer
        ).run(DestroyerDestructionContracts())
        destroyer_holder.destroy_particle()
        literal.record_operation("middle_b.destroy(destroyer_holder)")
        destruction_contracts.run_destructors_position_run(
            self.get_interface_position(
                "position<run>"
            ).particle
        )
        destruction_contracts.destroy_position_run(
            self.get_interface_position(
                "position<run>"
            ).particle
        )
        self.get_interface_position(
            "position<run>"
        ).destroy_particle()
        literal.record_operation("middle_b.destroy(run)")


class DestroyerDestructionContracts(local.my_domain_com.my_lib.destroyer.DestroyerDestructionContracts):

    @override
    def destroy_position_run(self, particle: literal.Particle):
        particle.get_position(
            local.my_domain_com.my_lib.sibling.Sibling
        ).destroy_particle()
        literal.record_operation("destroyer.destroy(run::/sibling)")
