"""Generated by the Define compiler."""

from typing import ClassVar, override

from define.runtime import literal

import local.my_domain_com.my_lib.a
import local.my_domain_com.my_lib.input


class OtherDestructionContracts:
    def run_destructors_global_position_input(self, _particle: literal.Particle):
        pass

    def destroy_global_position_input(self, _particle: literal.Particle):
        pass

    def run_destructors_global_position_input__position_a(self, _particle: literal.Particle):
        pass

    def destroy_global_position_input__position_a(self, _particle: literal.Particle):
        pass


_DEFAULT_DESTRUCTION_CONTRACTS = OtherDestructionContracts()


class Other(literal.Action):
    implied_qualities: ClassVar[tuple[type[literal.Quality], ...]] = (
        local.my_domain_com.my_lib.input.Input,
    )

    def __init__(self, on_particle: literal.Particle):
        super().__init__(
            on_particle,
            interface_positions=[
                literal.LocalPosition("position<holder_a>"),
                literal.LocalPosition("position<holder_b>"),
                literal.LocalPosition("position<holder_c>"),
                literal.LocalPosition("position<trigger_pos>"),
            ],
        )

    @override
    def run(self, destruction_contracts: OtherDestructionContracts = _DEFAULT_DESTRUCTION_CONTRACTS):
        self.on_particle.get_position(
            local.my_domain_com.my_lib.input.Input
        ).particle.get_position(
            local.my_domain_com.my_lib.a.A
        ).move_particle_to(
            self.get_interface_position(
                "position<holder_a>"
            )
        )
        literal.record_operation("other.move(/input::/a, holder_a)")
        self.get_interface_position(
            "position<holder_a>"
        ).move_particle_to(
            self.get_interface_position(
                "position<holder_b>"
            )
        )
        literal.record_operation("other.move(holder_a, holder_b)")
        self.on_particle.get_position(
            local.my_domain_com.my_lib.input.Input
        ).particle.get_position(
            local.my_domain_com.my_lib.a.A
        ).create_particle()
        literal.record_operation("other.create(/input::/a)")
        self.on_particle.get_position(
            local.my_domain_com.my_lib.input.Input
        ).particle.get_position(
            local.my_domain_com.my_lib.a.A
        ).destroy_particle()
        literal.record_operation("other.destroy(/input::/a)")
        self.get_interface_position(
            "position<holder_b>"
        ).move_particle_to(
            self.on_particle.get_position(
                local.my_domain_com.my_lib.input.Input
            ).particle.get_position(
                local.my_domain_com.my_lib.a.A
            )
        )
        literal.record_operation("other.move(holder_b, /input::/a)")
        self.on_particle.get_position(
            local.my_domain_com.my_lib.input.Input
        ).particle.get_position(
            local.my_domain_com.my_lib.a.A
        ).move_particle_to(
            self.get_interface_position(
                "position<holder_c>"
            )
        )
        literal.record_operation("other.move(/input::/a, holder_c)")
        destruction_contracts.run_destructors_global_position_input(
            self.on_particle.get_position(
                local.my_domain_com.my_lib.input.Input
            ).particle
        )
        destruction_contracts.destroy_global_position_input(
            self.on_particle.get_position(
                local.my_domain_com.my_lib.input.Input
            ).particle
        )
        self.on_particle.get_position(
            local.my_domain_com.my_lib.input.Input
        ).destroy_particle()
        literal.record_operation("other.destroy(/input)")
        destruction_contracts.run_destructors_global_position_input__position_a(
            self.get_interface_position(
                "position<holder_c>"
            ).particle
        )
        destruction_contracts.destroy_global_position_input__position_a(
            self.get_interface_position(
                "position<holder_c>"
            ).particle
        )
        self.get_interface_position(
            "position<holder_c>"
        ).destroy_particle()
        literal.record_operation("other.destroy(holder_c)")
