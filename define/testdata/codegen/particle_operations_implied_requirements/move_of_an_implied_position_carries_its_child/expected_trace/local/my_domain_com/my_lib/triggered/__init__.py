"""Generated by the Define compiler."""

from typing import ClassVar, override

from define.runtime import literal

import local.my_domain_com.my_lib.child
import local.my_domain_com.my_lib.implied


class TriggeredDestructionContracts:
    def run_destructors_global_position_implied__position_child(self, _particle: literal.Particle):
        pass

    def destroy_global_position_implied__position_child(self, _particle: literal.Particle):
        pass

    def run_destructors_position_run(self, _particle: literal.Particle):
        pass

    def destroy_position_run(self, _particle: literal.Particle):
        pass


_DEFAULT_DESTRUCTION_CONTRACTS = TriggeredDestructionContracts()


class Triggered(literal.Action):
    implied_qualities: ClassVar[tuple[type[literal.Quality], ...]] = (
        local.my_domain_com.my_lib.implied.Implied,
    )

    def __init__(self, on_particle: literal.Particle):
        super().__init__(
            on_particle,
            interface_positions=[
                literal.LocalPosition(
                    "position<dest>",
                    constraints=(
                        local.my_domain_com.my_lib.child.Child,
                    ),
                ),
                literal.LocalPosition("position<run>"),
            ],
        )

    @override
    def run(self, destruction_contracts: TriggeredDestructionContracts = _DEFAULT_DESTRUCTION_CONTRACTS):
        self.on_particle.get_position(
            local.my_domain_com.my_lib.implied.Implied
        ).move_particle_to(
            self.get_interface_position(
                "position<dest>"
            )
        )
        literal.record_operation("triggered.move(/implied, dest)")
        destruction_contracts.run_destructors_global_position_implied__position_child(
            self.get_interface_position(
                "position<dest>"
            ).particle.get_position(
                local.my_domain_com.my_lib.child.Child
            ).particle
        )
        destruction_contracts.destroy_global_position_implied__position_child(
            self.get_interface_position(
                "position<dest>"
            ).particle.get_position(
                local.my_domain_com.my_lib.child.Child
            ).particle
        )
        self.get_interface_position(
            "position<dest>"
        ).particle.get_position(
            local.my_domain_com.my_lib.child.Child
        ).destroy_particle()
        literal.record_operation("triggered.destroy(dest::/child)")
        destruction_contracts.run_destructors_position_run(
            self.get_interface_position(
                "position<run>"
            ).particle
        )
        destruction_contracts.destroy_position_run(
            self.get_interface_position(
                "position<run>"
            ).particle
        )
        self.get_interface_position(
            "position<run>"
        ).destroy_particle()
        literal.record_operation("triggered.destroy(run)")
