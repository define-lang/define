...
that is located in `foo/bar.dfn` relative to the project root? What if I load
`foo/bar.dfn` and I find something else in that file?

### 2: Sub-Roots Create Ambiguity

Imagine you have a project with a sub-root in `lib/math-utils` with a file
`lib/math-utils/adder.dfn`. You could refer to the code in `adder.dfn` in two
...
```
my_project/foo.dfn
my_project/lib/math_utils/adder.dfn
my_project/lib/math_utils/bar.dfn
```
...
What if `bar.dfn` is not actually part of `lib/math-utils` and it contains a
definition like:
...
decides to start including a file of its own called `bar.dfn` in future
...
Assuming that each `.dfn` file above defines a quality, the above example
...
error. For example, if `my_project/src/foo.dfn` contained `quality<fqun:/baz>`,
...
definitions. For example, `my_project/src/foo.dfn` might contain a definition
...
my_project/foo.dfn
my_project/lib/math_utils/adder.dfn
my_project/lib/math_utils/extra/bar.dfn
```

Now imagine the following sequence of events:

1. `foo.dfn` references
   `quality<example.com:my_project:/lib/math_utils/extra/bar>`.
2. The compiler loads and compiles `lib/math-utils/extra/bar.dfn`. At this time,
   no error is thrown.
3. `foo.dfn` then later references `quality<mv:alice.com:math_utils:/adder>`.
...
5. The compiler sees that it should load `adder.dfn` from `lib/math_utils`.
6. The compiler realizes that the earlier `bar.dfn` is contained inside of a
...
file `lib/math-utils/adder.dfn` must now say:
