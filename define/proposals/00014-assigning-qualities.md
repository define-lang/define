...
We need syntax for assigning qualities to particles.

## Solution

The syntax will be:

`assign the quality<name> to the particle in position<x>`

We are not yet defining what happens if you do this more than once to the same
particle. For now, this proposal assumes that you can only do this once to a
particle. That will likely change in the future, as points can logically have
...
Do we really need "to the particle?" I think we do, unfortunately. There may be
other syntax in the future that allows specifying constraints on _positions_,
and I think the language should make it clear that you're not changing the
position, you're changing the particle. You could move the point to another
position and it would still be the same particle, and its qualities would go
with it.
...
space). One is assigning the quality to the particle. Is this pedantic?
Probably. My present belief is that saying "to the particle" helps with
intuitive understanding.

#### One Line

We could have done: `create a particle in position<foo> with quality<bar>`.

However, that removes the possibility of empty positions and particles with no
quality, or it requires us to have more than one way to do the same thing.
...
define the position<foo> {
    ...
    the particle has the quality<bar>.
}
```
...
[DLP 13](00013-creating-particles.md). It's actually even worse, here: it
