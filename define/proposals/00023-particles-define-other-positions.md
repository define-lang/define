# Define Language Proposal 23: Particles Define Other Positions
...
be atomic, and that we will "assign" actions or other positions to particles.
This involved a decision that was hidden behind the scenes:

Can particles actually define the location of other particles, or should we just
limit ourselves to some concept of a form, and only forms can define groups of
particles?
...
1. Positions are inherently defined by the location of other particles. The
   first particle ever defined in a program is essentially the "view point" and
   all other particles are defined in relation to it. Thus, positions define
...
In the way we think about it, `balance` and `account_id` are particles with
value. You then have an action named `withdraw` and an action named `deposit`.
Thus `BankAccount` is both a form (it contains multiple particles) and a set of
qualities (it has two actions). That sounds like two separate concepts, right? A
form and a set of qualities. Well, the problem is, all these things are
inherently tied together. Let's look at this example as code to see the problem
more concretely (in an imaginary Python-like syntax):
...
wanted to update state on a particle.
...
"this particle" inside of an action, or not.)
...
abstract concept about a set of particles. It's the particles that exist, not
the forms. Most simply: In the universes I can logically consider, a machine
triggers based on particles that occupy specific _positions in space_, and it
affects particles in specific positions in space.

## Forward Compatibility

Once we decide to allow actions to reference "this particle", we can't convert
to being a pure functional programming language. That's the primary forward
