# Define Language Proposal 25: Dimension Points May Not Contain Themselves
...
move the dimension point in position<x> to position<x>::position<y>.
```
...
It is an error to move a dimension point into any position that it defines. The
...
`move the dimension point in position<x>::position<y> to position<x>::position<y>::position<z>.`

No matter how deep the nesting, if the position itself is the prefix of the
name, the dimension point in that position may not be moved there.
...
defined by the location of a dimension point in `position<x>`.
