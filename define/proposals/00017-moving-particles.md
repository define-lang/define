# Define Language Proposal 17: Moving Particles
...
motion means "a particle moves to another position."

One could imagine a universe instead where you never move particles and only
create new copies of them in new positions. This universe would quickly become
pretty static, though, as you'd never be able to relocate anything you made,
ever.

Thus, we need a syntax for moving particles.

## Solution

Similar to our creation syntax, the syntax for moving particles will be:
...
Note that it is an error to attempt to move a particle into the position it is
currently in (that doesn't make any sense).

It is an error to move a particle into a position already occupied by a
particle.
...
particles all at once, but we don't need it yet.

We could have removed "the particle in" but then the sentence wouldn't really be
_true_. You're not moving the position. I think that actually would be confusing
for programmers as they read it.
