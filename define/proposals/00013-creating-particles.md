# Define Language Proposal 13: Creating Particles
...
We need syntax for creating particles.

We have no way to refer to particles other than by their position.
...
concepts in the universe of reflection, but particles exist in the universe of
the program.

Thus, our syntax for creating a particle looks like this:
...
It is an error to create a particle in a position already occupied by a
particle.
...
particle. This is because of the "one right way" principle of Define. This is
the only way to allow the options of creating empty positions _and_ filled
positions, but keeping the syntax totally consistent either way.
...
We could have simply just done: `create a particle in position<foo>`. Many of
the examples written before this proposal do exactly that, for the sake of
simplicity. However, that removes the possibility of defining empty positions.
...
- It doesn't allow you to easily create the particle much later than you define
  the position, in the program. That may or may not be necessary; I haven't yet
...
```
define the position<foo>.
position<foo> now contains a particle.
```
