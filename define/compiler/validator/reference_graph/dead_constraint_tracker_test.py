...
def _dead_keys(
    ...
    return {candidate.key for candidate in tracker.dead_constraints()}


def test_empty_tracker_has_no_pending():
    tracker = dead_constraint_tracker.DeadConstraintTracker()
...
def test_referenced_child_position_is_alive():
    ...
    thing = _global_ref("/thing")
    tracker.register_constraint(box, thing)
    tracker.mark_alive(_position_ref(box, thing))
    assert _dead_keys(tracker) == set()
...
def test_reference_to_action_child_does_not_mark_it_alive():
    ...
    tracker.mark_alive(_position_ref(box, coin, _local_ref("go")))
...
def test_triggered_action_is_alive():
    ...
    coin = _global_ref("/coin", ast.NameType.ACTION)
    tracker.register_constraint(box, coin)
    tracker.mark_alive(_action_ref(box, coin))
    assert _dead_keys(tracker) == set()
...
def test_mark_constraints_alive_clears_a_positions_candidates():
    tracker = dead_constraint_tracker.DeadConstraintTracker()
    ...
    tracker.mark_constraints_alive(iface.typed_name, iface.constraint_typed_names)
    assert _dead_keys(tracker) == set()
...
def test_one_constraint_alive_while_a_sibling_stays_dead():
    ...
    tracker.mark_alive(_position_ref(box, a))
