...
- Use the `parse` fixture (type `Parse` from `conftest`). It asserts
  `diagnostics == []`, raises any parse exception, and returns the tree. Use
  `pytest.raises` for tests that expect a parse error. Only use the `p` fixture
  when you need access to the raw `ParseResult` (e.g. to check non-empty
  diagnostics).
