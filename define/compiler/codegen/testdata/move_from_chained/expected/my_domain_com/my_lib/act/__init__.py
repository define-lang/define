"""Generated by the Define compiler."""

from typing import ClassVar, override

from define.runtime import literal

import my_domain_com.my_lib.mid_dest
import my_domain_com.my_lib.mid_src_a
import my_domain_com.my_lib.mid_src_b
import my_domain_com.my_lib.mid_src_c


class Act(literal.Action):
    _typed_name: ClassVar[str] = "action<my.domain.com:my_lib:/act>"

    def __init__(self):
        super().__init__(
            interface_positions=[
                literal.InterfacePosition("position<trigger>"),
                literal.InterfacePosition(
                    "position<chain_src_a>",
                    constraints=[
                        my_domain_com.my_lib.mid_src_a.MidSrcA,
                    ],
                ),
                literal.InterfacePosition(
                    "position<chain_src_b>",
                    constraints=[
                        my_domain_com.my_lib.mid_src_b.MidSrcB,
                    ],
                ),
                literal.InterfacePosition(
                    "position<chain_src_c>",
                    constraints=[
                        my_domain_com.my_lib.mid_src_c.MidSrcC,
                    ],
                ),
                literal.InterfacePosition("position<iface_dest>"),
                literal.InterfacePosition(
                    "position<chain_dest>",
                    constraints=[
                        my_domain_com.my_lib.mid_dest.MidDest,
                    ],
                ),
            ],
            trigger_position_name="position<trigger>",
        )

    @override
    def execute(self):
        local_dest = literal.LocalPosition("position<local_dest>")
        self.get_interface_position(
            "position<chain_src_a>"
        ).dimension_point.get_position(
            "position<my.domain.com:my_lib:/mid_src_a>"
        ).dimension_point.get_position(
            "position<my.domain.com:my_lib:/end_src_a>"
        ).move_dimension_point_to(local_dest)
        self.get_interface_position(
            "position<chain_src_b>"
        ).dimension_point.get_position(
            "position<my.domain.com:my_lib:/mid_src_b>"
        ).dimension_point.get_position(
            "position<my.domain.com:my_lib:/end_src_b>"
        ).move_dimension_point_to(
            self.get_interface_position(
                "position<iface_dest>"
            )
        )
        self.get_interface_position(
            "position<chain_src_c>"
        ).dimension_point.get_position(
            "position<my.domain.com:my_lib:/mid_src_c>"
        ).dimension_point.get_position(
            "position<my.domain.com:my_lib:/end_src_c>"
        ).move_dimension_point_to(
            self.get_interface_position(
                "position<chain_dest>"
            ).dimension_point.get_position(
                "position<my.domain.com:my_lib:/mid_dest>"
            ).dimension_point.get_position(
                "position<my.domain.com:my_lib:/end_dest>"
            )
        )
