...
def _generated_action(
    ...
    execution = template_context.ActionExecutionContext(
        ...
        [],
        [],
        template_context.ActionTriggersContext([], [], []),
        None,
    )
...
def _action_names(
    ...
    *,
    callees_have_guarantees: bool = False,
    ...
    for action_trigger in plan.action_triggers:
        guarantee_interface = None
        if callees_have_guarantees:
            guarantee_interface = action_context.GuaranteeInterface(
                naming.ClassReference("CalleeGuarantees", "local.callee"),
                {},
                {},
            )
        generated_actions[action_trigger.callee_action_name] = _generated_action(
            guarantee_interface
        )
...
def test_repeated_action_trigger_skips_a_source_suffix(
    ...
    names = _action_names(plan, callees_have_guarantees=True)
    ...
    assert names.child_guarantees == {
        naturally_suffixed: "trigger_position_gateway__global_action_worker_2",
        first: "trigger_position_gateway__global_action_worker",
        second: "trigger_position_gateway__global_action_worker_3",
    }
