...
class TestRun:
    ...
    def test_compile_writes_generated_code_to_code_output_stream(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.chdir(PROJECTS_ROOT / "valid" / "position_definition")
        ...
        result = driver.Driver().run(
            ...
            code_output_stream=code_stream,
        )
        assert result == driver.ExitCode.SUCCESS
        output = code_stream.getvalue()
        assert "Generated by the Define compiler" in output
        assert "class Test(literal.GlobalPosition)" in output

    def test_compile_with_errors_does_not_write_code(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.chdir(FILES_ROOT)
        code_stream = io.StringIO()
        ...
        assert result == driver.ExitCode.ERROR
        assert code_stream.getvalue() == ""

    def test_validate_mode_does_not_write_code(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.chdir(PROJECTS_ROOT / "valid" / "position_definition")
        code_stream = io.StringIO()
        result = driver.Driver().run(
            Path("test.def"),
            mode=driver.DriverMode.VALIDATE,
            code_output_stream=code_stream,
        )
        assert result == driver.ExitCode.SUCCESS
        assert code_stream.getvalue() == ""
