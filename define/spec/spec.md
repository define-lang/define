...
the same as for a local position, except that it may begin with one or more
...
the action's Action Statements Block or Trigger Conditions Block. If the quality
is not so referenced, the compiler must throw an error.
...
Any constructor may be the entry point of a program. Here is what logically
occurs from the compiler's viewpoint when starting a program:
